...
`og_image` を設定すると、avel は `og:image`、`og:image:secure_url`、`twitter:image`、大きな Twitter Card を出力します。`og_image_alt` を設定すると、Open Graph と Twitter の画像 alt に使われます。省略した場合は現在のページタイトルが使われます。`og_image_type`、`og_image_width`、`og_image_height` は省略できますが、SNS クローラが画像プレビューを安定して描画する助けになります。
