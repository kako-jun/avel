...
## ステップ 3 — config.toml に 3 項目追加

```toml
[extra]
nostalgic_bbs = true
nostalgic_bbs_limit = 3          # 画像に表示する最新コメント数
nostalgic_bbs_width = 760        # SVG の描画幅（px）
nostalgic_bbs_label = "Comments" # 画像の上に出る見出し
nostalgic_bbs_hint = "クリックで書き込めます"  # 画像の下のキャプション
```
